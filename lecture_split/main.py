...
def detect_scenes(video_pth: Path, scene_detection_threshold: int, min_scene_length: int,
    ...
    print(f"Starting detect_scene with args: \n"
    ...
    try:
        ...
        print(f"Processing complete in {round(time_taken, 1)} seconds")
...
def extract_split_audio(video_pth: Path, output_dir: Path, scene_list: List[Tuple[FrameTimecode, FrameTimecode]],
    ...
    for i, (start_time, end_time) in tqdm(enumerate(scene_list), total=len(scene_list), unit="scene", miniters=1,
        ...
        if not suppress_output and i == 0 and len(scene_list) > 1:
            print('Output from ffmpeg for Scene 1 shown above, splitting remaining scenes...')
        if ret_val != 0:
            break
...
def main():
    ...
    parser.add_argument('frame_skip',
                        ...
                        default=5, type=int)
    # INPUT_FILE = PROJECT_DIR / "tests" / "testdata" / "lecture_trimmed.mp4"
    # OUTPUT_DIR = PROJECT_DIR / "output" / INPUT_FILE.stem
    # SCENE_DETECTION_THRESHOLD = 5
    # MIN_SCENE_LENGTH = 100  # In frames, default 100
    ...
    print(f"All processing complete, please check output files in {Path(args.output_dir).absolute()}")
